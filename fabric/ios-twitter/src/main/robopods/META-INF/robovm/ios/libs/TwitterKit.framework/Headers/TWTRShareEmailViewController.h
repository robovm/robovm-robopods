 ...
 */
typedef void (^TWTRShareEmailCompletion)(NSString * __ email, NSError * __  error);
 ...
 *  @param completion The completion block called when the user either accepts or denies access to their email address. Called on the main thread.
 */
- (instancetype)initWithUserID:( NSString *)userID completion:(  TWTRShareEmailCompletion)completion;
